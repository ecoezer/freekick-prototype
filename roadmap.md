...
## 🎯 Aşama 2: Nişan Alma ve UI (Yakında)
- [ ] **Hedef Belirleme:** `AimDirectionProvider` eklenerek oyuncunun ekranda nişan aldığı noktaya doğru şut atabilmesi.
- [ ] **Görsel Yardımcılar:** Topun gidiş yönünü gösteren LineRenderer (`AimVisualController`).
- [ ] **UI Güç Barı:** `ShotInput.OnPowerChanged` eventini dinleyerek dolan bir güç barı arayüzü (`PowerBarView`).
- [ ] **Gol Tespiti:** Kaledeki görünmez collider ile çarpışmayı algılayan `GoalDetector` scriptinin yazılması.
